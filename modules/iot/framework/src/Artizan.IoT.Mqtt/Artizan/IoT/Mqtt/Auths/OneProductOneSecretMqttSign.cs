...
public class OneProductOneSecretMqttSign
{
    ...
    public MqttAuthResult ValidateSign(string clientId, string userName, string password, string productSecret, MqttAuthParams? authParams = null)
    {
        try
        {
            ...
            if (authParams == null)
            {
                return MqttAuthResult.Fail(MqttAuthErrorCode.ClientIdFormatInvalid, "ClientId格式非法（需符合：DeviceName|authType=xxx,...|）");
            }

            // 2. 验证认证类型匹配
            if (authParams.AuthType != _authType)
            {
                return MqttAuthResult.Fail(MqttAuthErrorCode.AuthTypeMismatch, $"认证类型不匹配，预期：{_authType}");
            }

            // 3. 强制验证安全模式（必须匹配固定值）
            if (authParams.SecureMode != _fixedSecureMode)
            {
                return MqttAuthResult.Fail(MqttAuthErrorCode.SecurityModeMismatch, $"安全模式不匹配，{_authType}强制要求：{_fixedSecureMode}");
            }
