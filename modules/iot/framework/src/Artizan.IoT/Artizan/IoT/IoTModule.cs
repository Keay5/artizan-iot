...
public class IoTModule : AbpModule
{
    ...
    public override async Task OnApplicationInitializationAsync(ApplicationInitializationContext context)
    {
        ...
        await StartMqttMessageDispatcherAsync(context);
    }

    public override async Task OnApplicationShutdownAsync(ApplicationShutdownContext context)
    {
        await StopMqttMessageDispatcherAsync(context);
    }
    ...
    private static void ConfigureMqttServices(ServiceConfigurationContext context)
    {
        ...
        #region MQTT 消息路由
        // 1. 注册MQTT 消息路由配置选项（支持通过appsettings.json配置）
        context.Services.Configure<MqttRouterOptions>(context.Configuration.GetSection("IoT:Mqtt:Router"));
        // 2. 注册后台服务：定时MQTT 消息路由清理解析缓存
        context.Services.AddHostedService<MqttTopicRouteCacheCleanupBackgroundService>();
        #endregion

        #region MQTT 消息分发器
        context.Services.Configure<MqttMessageDispatcherOptions>(context.Configuration.GetSection("IoT:Mqtt:MessageDispatcher"));
        #endregion
    ...
    private async Task StartMqttMessageDispatcherAsync(ApplicationInitializationContext context)
    {
        var dispatcher = context.ServiceProvider.GetRequiredService<IMqttMessageDispatcher>();
        await dispatcher.StartAsync();
    }

    private async Task StopMqttMessageDispatcherAsync(ApplicationShutdownContext context)
    {
        var dispatcher = context.ServiceProvider.GetRequiredService<IMqttMessageDispatcher>();
    ...
    private static void ConfigureThingCacheServices(ServiceConfigurationContext context)
    {
        ...
        //// 1. 从配置文件读取Redis连接字符串
        //var redisConnString = configuration.GetSection("IoT:Cache:Redis:ConnectionString").Value;
        //if (string.IsNullOrEmpty(redisConnString))
        //{
        //    throw new AbpException("Redis连接字符串未配置，请检查appsettings.json");
        //}
        //// 3. 创建连接并注册为单例（ConnectionMultiplexer 线程安全，推荐全局单例）
        ...
        // 注册Redis连接复用器（单例，避免重复创建连接）
        //services.AddSingleton<IConnectionMultiplexer>(sp =>
        //{
        //    var configuration = sp.GetRequiredService<IConfiguration>();
        //    var redisConnStr = configuration.GetSection("IoT:Cache:Redis:ConnectionString").Value;
