...
/// </summary>
public class ThingPropertyHistoryDataCacheManager : IThingPropertyHistoryDataCacheManager
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // 生成缓存键和分数（时间戳转毫秒级）
        var cacheKey = cacheItem.CalculateHistoryDataCacheKey();
    ...
    /// 按时间范围查询历史属性数据
    /// </summary>
    public async Task<(IList<ThingPropertyDataCacheItem> Items, int TotalCount)> GetByTimeRangeAsync(
        string productKey,
        string deviceName,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // 生成缓存键并查询
        var cacheKey = ThingPropertyDataCacheItem.CalculateHistoryDataCacheKey(productKey, deviceName, propertyIdentifier);
    ...
    /// </summary>
    public async Task<(IList<ThingPropertyDataCacheItem> Items, int TotalCount)> GetByRecentMinutesAsync(
        string productKey,
        string deviceName,
        string propertyIdentifier,
        int minutes,
    ...
    /// 删除指定时间范围内的历史属性数据
    /// </summary>
    public async Task RemoveByTimeRangeAsync(
        string productKey,
        string deviceName,
        string propertyIdentifier,
        DateTime startTime,
    ...
    /// 清理过期的历史数据
    /// </summary>
    public async Task CleanupExpiredHistoryAsync(CancellationToken cancellationToken = default)
    {
        // 计算过期时间（当前时间 - 保留时长）
        ...
        // 获取所有历史数据缓存键前缀
        var historyPrefix = "thing:cache:history:";
