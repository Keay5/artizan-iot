...
/// </summary>
public class ThingPropertyDataCacheManager : 
    ThingPropertyDataCacheManagerBase, 
    IThingPropertyDataCacheManager
{
    ...
    /// 获取单个设备属性的最新缓存值
    /// </summary>
    public override async Task<ThingPropertyDataCacheItem?> GetAsync(
        string productKey,
        string deviceName,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var cacheKey = BuildQualifiedCacheKey(ThingPropertyDataCacheItem.CalculateLatestDataCacheKey(productKey, deviceName, propertyIdentifier));
    ...
    /// 批量获取设备属性的最新缓存值
    /// </summary>
    public override async Task<IDictionary<string, ThingPropertyDataCacheItem>> GetManyAsync(
        string productKey,
        string deviceName,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var keyMap = propertyIdentifiers.ToDictionary(
            pid => pid,
            pid => BuildQualifiedCacheKey(ThingPropertyDataCacheItem.CalculateLatestDataCacheKey(productKey, deviceName, pid)));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        //cacheItem.Version = cacheItem.Version + 1;
        ...
        // 生成缓存键并调用存储提供者
        var cacheKey = BuildQualifiedCacheKey(cacheItem.CalculateLatestDataCacheKey());
    ...
    /// 批量设置设备属性的最新缓存值
    /// </summary>
    public override async Task SetManyAsync(
    ...
    /// 批量移除设备属性的缓存
    /// </summary>
    public override async Task RemoveManyAsync(
        string productKey,
        string deviceName,
        IEnumerable<string> propertyIdentifiers,
        CancellationToken cancellationToken = default)
    {
        ...
        var cacheKeys = propertyIdentifiers
            .Select(pid => BuildQualifiedCacheKey(ThingPropertyDataCacheItem.CalculateLatestDataCacheKey(productKey, deviceName, pid)))
            .ToList();
    ...
    /// 移除指定设备的所有属性缓存
    /// </summary>
    public override async Task RemoveDeviceAllPropertiesAsync(
        string productKey,
        string deviceName,
        CancellationToken cancellationToken = default)
    {
        ...
        // 获取设备维度缓存前缀，批量删除
        var prefix = BuildQualifiedCacheKey(ThingPropertyDataCacheItem.CalculateDeviceLatestDataPrefix(productKey, deviceName));
