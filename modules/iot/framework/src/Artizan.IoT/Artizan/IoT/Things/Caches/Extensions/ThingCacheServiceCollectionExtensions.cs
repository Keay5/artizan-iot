...
public static class ThingCacheServiceCollectionExtensions
{
        ...
        Action<ThingPropertyHistoryDataCacheOptions>? configureHistoryOptions = null)
    {
        ...
        //        CacheStorageType.Redis => CreateRedisStorageProvider(sp, latestOptions),
        //        // 本地内存存储：注册ThingLocalMemoryCacheProvider
        //        CacheStorageType.LocalMemory => new ThingLocalMemoryCacheProvider(),
        ...
        services.Configure<ThingPropertyDataCacheOptions>(configuration.GetSection("IoT:Cache:ThingPropertyData"));
        services.Configure<ThingPropertyHistoryDataCacheOptions>(configuration.GetSection("IoT:Cache:ThingPropertyHistoryData"));

        var cacheStorageType = configuration["IoT:Cache:StorageType"] ?? throw new AbpException("IoT Cache 配置项 IoT:Cache:StorageType 未找到");
        if (cacheStorageType.ToLower() == CacheStorageType.Redis.ToString().ToLower())
        {
            services.AddSingleton<IThingCacheStorageProvider>(sp =>
            {
                var connection = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]);
                return new ThingRedisCacheProvider(connection);
            });
        }
        else if(cacheStorageType.ToLower() == CacheStorageType.LocalMemory.ToString().ToLower())  
        {
            services.AddSingleton<IThingCacheStorageProvider>(sp =>
            {
                return new ThingLocalMemoryCacheProvider();
            });
        }
        else 
        {
            throw new NotSupportedException($"不支持的缓存存储类型：{cacheStorageType}");
        }
