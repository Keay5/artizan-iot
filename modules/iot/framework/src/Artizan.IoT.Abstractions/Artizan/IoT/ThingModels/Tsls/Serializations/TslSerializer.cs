...
/// </summary>
public static class TslSerializer
{
    public static readonly JsonSerializerSettings DefaulJsonSerializerSettings = new()
    {
        ContractResolver = new OrderedContractResolver(), // 结合代码中 OrderedContractResolver 的实现（OrderBy(p => p.Order ?? int.MaxValue)），可使序列化后的 JSON 属性按 [JsonProperty(Order = x)] 标记的顺序排列，未标记的属性默认排在最后，确保 JSON 结构的有序性。
        NullValueHandling = NullValueHandling.Ignore,// None 表示序列化后的 JSON 为紧凑格式（无缩进、无换行），适合网络传输或存储
