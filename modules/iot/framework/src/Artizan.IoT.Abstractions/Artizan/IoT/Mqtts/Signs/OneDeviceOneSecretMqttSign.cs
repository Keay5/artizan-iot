...
public class OneDeviceOneSecretMqttSign
{
    ...
    public MqttAuthResult ValidateSign(string mqttClientId, string mqttUserName, string mqttPassword, string deviceSecret, MqttAuthParams? authParams = null)
    {
        try
        {
            ...
            if (authParams == null)
            {
                return CreateFailResult(2, "ClientId格式非法（需符合：ProductKey.DeviceName|authType=xxx,...|）");
            }

            // 2. 验证认证类型匹配
            if (authParams.AuthType != MqttAuthType.OneDeviceOneSecret)
            {
                return CreateFailResult(2, $"认证类型不匹配，预期：{MqttAuthType.OneDeviceOneSecret}");
            }
            ...
            {
                return CreateFailResult(2, "ClientId前缀格式非法（需符合：ProductKey.DeviceName）");
            }
