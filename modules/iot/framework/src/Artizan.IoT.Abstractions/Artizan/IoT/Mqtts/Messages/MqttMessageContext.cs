using Artizan.IoT.Commons.Extensions;
...
public class MqttMessageContext : MessageContext
{
    ...
    /// <summary>
    /// 消息解析类型（标记ParsedData的数据格式，避免类型转换错误）
    /// 设计：与ParsedData配合，避免类型转换错误
    /// </summary>
    public MqttMessageDataParseType ParseType { get; private set; } = MqttMessageDataParseType.Unknown;
    ...
    //[Obsolete("推荐使用基础构造函数，通过TopicPlaceholderValues赋值ProductKey/DeviceName，提升扩展性")]
    //public MqttMessageContext(
    //    MqttRawMessage mqttRawMessage,
    //    string mqttClientId,
    //    string productKey,
    ...
    /// </summary>
    /// <param name="stepName">步骤名称（非空）</param>
    ...
    public void UpdateStepResult(string stepName, bool isSuccess, TimeSpan elapsed, string? errorMsg = null, Exception? exception = null)
    {
        ...
        {
            throw new ArgumentException($"[{TraceId}] 步骤名称不能为空", nameof(stepName));
        }

        if (!isSuccess && string.IsNullOrWhiteSpace(errorMsg))
        {
            throw new ArgumentException($"[{TraceId}] 失败步骤[{stepName}]必须提供错误信息", nameof(errorMsg));
        }

        // 构建步骤结果（UTC时间戳，统一时间基准）
        var stepResult = new MqttMessageProcessStepExecuteResult
        {
            StepName = stepName,
            IsSuccess = isSuccess,
            Elapsed = elapsed,
        ...
        _stepResults[stepName] = stepResult;
    }
    ...
    /// <exception cref="ObjectDisposedException">对象已释放时抛出</exception>
    public IReadOnlyDictionary<string, MqttMessageProcessStepExecuteResult> GetAllStepResults()
    {
        CheckDisposed();
        return _stepResults;
    }
