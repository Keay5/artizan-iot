...
public class MqttMessageDispatcher : IMqttMessageDispatcher, ISingletonDependency
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        _isStarted = true;
        _logger.LogInformation("[MQTT消息分发器] 启动完成 | 分区数：{PartitionCount} | Channel容量：{ChannelCapacity}",
            _options.PartitionCount, _options.ChannelCapacity);
    }
    ...
    //            // 记录异常日志（关联TraceId）
    //            _logger.LogError(ex, "[{TraceId}][MQTT消息分发器] 消息处理失败", context.TraceId);
    //        }
    //        finally
    //        {
    ...
    ///// <summary>
    ///// 计算分区键（按设备哈希取模）（哈希取模，确保均匀分布）
    ///// 这个算法的设计初衷是保证 “同一个设备的消息严格按顺序处理”（比如设备上报的时序数据不能乱序），
    ///// 这在 “设备数量多、单设备消息量小” 的场景下是合理的 
    ///// —— 不同设备的消息分散到不同分区，整体能利用多分区并行，
    ///// 既保证单设备顺序，又兼顾性能。
