...
public class RuleMatchMqttTopicPermissionValidator : IMqttTopicPermissionValidator, ISingletonDependency
{
    ...
    public async Task<MqttTopicPermissionResult> ValidateAsync(MqttTopicPermissionContext context)
    {
            ...
            switch (compareResult)
            {
                case MqttTopicFilterCompareResult.IsMatch:
                    ...
                    else
                    {
                        ...
                        _logger.LogWarning("[{TrackId}][MQTT Topic 权限] {DenyReason}", context.TrackId, denyReason);
                ...
                case MqttTopicFilterCompareResult.FilterInvalid:
                    _logger.LogWarning("[{TrackId}][MQTT Topic 权限] 规则模板非法 | Identifier={Identifier} | Pattern={Pattern}",
                        context.TrackId, rule.Identifier, rule.TopicPattern);
                    continue;

                case MqttTopicFilterCompareResult.TopicInvalid:
                    var topicInvalidReason = $"待校验Topic非法 | Topic={context.Topic}";
                    _logger.LogWarning("[{TrackId}][MQTT Topic 权限] {topicInvalidReason}", context.TrackId);
                    return new MqttTopicPermissionResult
                    {
                        IsAllowed = false,
        ...
        if (hasAllowRule)
        {
            _logger.LogDebug("[{TrackId}][MQTT Topic 权限] 匹配允许规则 | Identifier={Identifier} | Topic={Topic}",
                context.TrackId, matchedAllowRule!.Identifier, context.Topic);
            return new MqttTopicPermissionResult
            {
                IsAllowed = true,
        ...
        {
            _logger.LogDebug("[{TrackId}][MQTT Topic 权限] 无匹配拒绝规则，默认允许 | Topic={Topic}",
                context.TrackId, context.Topic);
            return new MqttTopicPermissionResult
            {
                IsAllowed = true,
                DenyReason = string.Empty
