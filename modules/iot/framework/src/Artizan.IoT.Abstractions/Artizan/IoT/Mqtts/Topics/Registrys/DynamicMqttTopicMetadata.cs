...
{
    /// <summary>
    /// MQTT Topic模板（支持${productKey}/${deviceName}格式占位符）
    /// </summary>
    ...
    public DynamicMqttTopicMetadata(string topicTemplate, Type handlerType, int priority = 0)
    {
        ...
        if (!typeof(IMqttMessageHandler).IsAssignableFrom(handlerType))
        {
    ...
    public IMqttMessageHandler CreateHandler(IServiceProvider serviceProvider)
    {
