    ...
    private readonly Dictionary<string, string> _alinkFieldMappings = new()
{
    ...
    public AlinkHandleResult Parse(string rawMessage, string rawTopic)
    {
        ...
        try
        {
            ...
            var standardMessage = new AlinkMessage(_traceIdAccessor)
            {
            ...
            {
                traceId = idValue;
                standardMessage.TraceId = traceId;
                // 同步到上下文，后续流程复用
            ...
            standardMessage.ProductKey = productKey;
            ...
            standardMessage.DeviceName = deviceName;
            ...
            standardMessage.Method = method;

            // 可选字段提取（缺失则赋默认值）
            if (TryGetJsonProperty(alinkObj, "params", out var paramStr))
            {
                // 统一转换为JSON字符串，兼容Params是原始值/对象的情况
                standardMessage.Params = paramStr.StartsWith("{") ? paramStr : JsonSerializer.Serialize(paramStr);
            }
            if (TryGetJsonProperty(alinkObj, "timestamp", out var timestampStr) && long.TryParse(timestampStr, out var timestamp))
            {
                standardMessage.Timestamp = timestamp;
            }

            _logger.LogInformation("[TraceId:{TraceId}] AlinkJson解析成功 | ProductKey={PK} | DeviceName={DN} | Method={Method}",
                traceId, standardMessage.ProductKey, standardMessage.DeviceName, standardMessage.Method);

            return AlinkHandleResult.Success(standardMessage);
        }
