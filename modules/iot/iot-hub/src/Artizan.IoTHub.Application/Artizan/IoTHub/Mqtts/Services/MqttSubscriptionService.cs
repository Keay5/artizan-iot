...
public class MqttSubscriptionService : MqttServiceBase, IMqttSubscriptionService, ISingletonDependency
{
    ...
    private async Task InterceptingSubscriptionHandlerAsync(InterceptingSubscriptionEventArgs eventArgs)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(signParams.ProductKey) || string.IsNullOrWhiteSpace(signParams.DeviceName))
            {
    ...
    private async Task ClientSubscribedTopicHandlerAsync(ClientSubscribedTopicEventArgs eventArgs)
    {
        ...
        MqttSignParams? signParams = eventArgs.SessionItems[signParamsSessionItemKey] as MqttSignParams;
        var productKey = signParams?.ProductKey;
        var deviceName = signParams?.DeviceName;
        ...
        try
        {
            ...
            Logger.LogInformation(
               ...
               trackId, signParams.ProductKey, signParams.DeviceName, topic
            );
        }
        catch (Exception ex)
        {
    ...
    private async Task ClientUnsubscribedTopicHandlerAsync(ClientUnsubscribedTopicEventArgs eventArgs)
    {
        ...
        Logger.LogInformation($"[{trackId}][MQTT Topic 订阅] 取消订阅主题成功，设备：{signParams?.ProductKey}/{signParams?.DeviceName}, 主题：{topic}, clientId:{eventArgs.ClientId}");
    }
}
