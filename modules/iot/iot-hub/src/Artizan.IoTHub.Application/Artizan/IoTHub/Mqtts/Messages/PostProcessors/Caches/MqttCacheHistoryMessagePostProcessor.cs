...
public class MqttCacheHistoryMessagePostProcessor :
    IMessagePostProcessor<MqttMessageContext>,
    ISingletonDependency
{
    private readonly ILogger<MqttCacheHistoryMessagePostProcessor> _logger;
    ...
    /// </summary>
    public int Priority => MqttMessagePostProcessorPriorityConstants.MqttCacheHistoryMessagePostProcessorPriority;
    ...
    public MqttCacheHistoryMessagePostProcessor(
        ILogger<MqttCacheHistoryMessagePostProcessor> logger,
    ...
    private async Task WriteThingPropertyDataToCacheAsync(MqttMessageContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (IsBatchTriggerConditionMet())
        {
            ...
            try
            {
                ...
                if (batch.Count > 0)
                {
                    _logger.LogInformation($"[{context.TraceId}][Thing Property 历史数据] 触发批量写入物模型属性历史缓存缓存，条数：{batch.Count}");
                    
                    // 批量写入缓存
                    await _propertyHistoryDataCacheManager.AddManyAsync(batch);
                }
