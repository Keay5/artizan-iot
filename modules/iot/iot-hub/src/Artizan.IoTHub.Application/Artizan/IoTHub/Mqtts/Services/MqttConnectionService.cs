...
public class MqttConnectionService : MqttServiceBase, IMqttConnectionService, ISingletonDependency
{
    ...
    /// <param name="thingIdentifier">设备唯一标识</param>
    private async Task ExecuteAuthenticationWithCircuitBreakerAsync(ValidatingConnectionEventArgs eventArgs, MqttAuthParams authParams, string thingIdentifier, string trackId)
    {
        // 获取或创建设备专属的熔断策略
    ...
    /// <exception cref="MqttAuthenticationFailedException">认证失败时抛出</exception>
    private async Task<MqttAuthValidationResult> HandleAuthenticationAsync(MqttAuthParams authParams, ValidatingConnectionEventArgs eventArgs, string trackId)
    {
        // TODO: 这里的业务可以考虑移动到 Domain Service 服务中，以便更好地进行单元测试和业务逻辑复用
        try
        {
            string secret;
        ...
        catch (MqttAuthenticationFailedException)
        {
        ...
        catch (Exception ex)
        {
            ...
            throw new MqttAuthenticationFailedException(errorMsg, ex);
        }
    }
